...
//				   << QVector3D(+2.0f, +1.0f, +0.0f)
	...
	void initializeGL(){
		...
		// Application-specific initialization
		{
			...
			m_vbo.allocate(sizeof(QVector3D)*m_vertices.size());
			m_vbo.write(0, m_vertices.constData(), sizeof(QVector3D)*m_vertices.size());
...
int main(int argc, char *argv[]){
	QSurfaceFormat fmt;
	qDebug() << "renderableType: " << fmt.renderableType();
	qDebug() << glGetString(GL_VERSION);
	fmt.setVersion(5,8);//versionを指定 対応してない高いバージョン指定すると自動的にバージョン下げられるっぽい。自PCでは4.4.0になる
